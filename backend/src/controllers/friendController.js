...
export const sendFriendRequest = async (req, res) => {
  try {
    ...
    if (existingRequest) {
      return res
        .status(400)
        .json({ message: "Đã có lời mời kết bạn đang chờ đồng ý" });
    }
...
export const declineFriendRequest = async (req, res) => {
  try {
    ...
    if (request.to.toString() !== userId.toString()) {
      return res
        .status(403)
        .json({ message: "Không có quyền chấp nhận lời mời kết bạn này" });
    }
...
export const getFriendRequests = async (req, res) => {
  try {
    ...
    console.error("Lỗi khi gọi getFriendRequests", error);
    return res.status(500).json({ message: "Lỗi hệ thống" });
  }
};
