...
const ChatCard = ({
  convoId, name, timestamp, isActive, onSelect, unreadCounts, leftSection, subtitle
}: ChatCardProps) => {
  return (
    <Card
      key={convoId}
        ...
        <div className='flex-1 min-w-0'>
          <div className='flex items-center justify-between mb-1'>
            <h3 className={cn("font-semibold text-sm truncate",
              unreadCounts && unreadCounts > 0 && "text-foreground")}>{name}</h3>
            <span className='text-xs text-muted-foreground'>{timestamp ? formatOnlineTime(timestamp) : ""}</span>
          </div>
          <div className='flex items-center justify-between'>
            <div className='flex items-center gap-1 flex-1 min-w-0'>{subtitle}</div>
            <MoreHorizontal className='siez-4 text-muted-foreground opacity-0 group-hover:opacity-100 hover:size-5 transition-smooth' />
          </div>
        </div>
      </div>
